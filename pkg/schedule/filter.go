...
type Filter interface {
	Name() string
	Filter(c context.Context, in PickInput, eps []*ctx.Endpoint) (kept []*ctx.Endpoint, rec ctx.FilterRecord)
}

// Scorer 在 Filter 之上加打分能力（如 PrefixCacheScheduler）。
type Scorer interface {
	Filter
	Score(c context.Context, in PickInput, eps []*ctx.Endpoint) []ScoredEndpoint
}
