...
type Checker interface {
	...
	BuildSpec(id ctx.UserIdentity, ms *ctx.ModelServiceSnapshot) *ctx.LimitSpec

	// CheckReadOnly 三层预检（用户层 + 模型层），read-only。
	// Endpoint 层不在此处检查（在调度层 LimitReadFilter 内做）。
	CheckReadOnly(c context.Context, spec *ctx.LimitSpec, id ctx.UserIdentity, ms *ctx.ModelServiceSnapshot) CheckResult
	...
	Consume(c context.Context, spec *ctx.LimitSpec, id ctx.UserIdentity, u *ctx.Usage) error
}
