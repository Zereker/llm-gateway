...
-- =====================================================================
-- data_change_outbox：admin 写库时**同事务**追加一行；relay goroutine 异步消费 → Redis
--
-- 这是 transactional outbox 模式（替代 Debezium 这类二进制 log 抓取），让控制面变更
-- 准实时传播到数据面：
--
--   admin POST /endpoints
--     → tx Begin
--     → INSERT endpoints
--     → INSERT data_change_outbox (table=endpoints, op=upsert, pk=<id>, payload=<row JSON>)
--     → tx Commit
--
--   admin OutboxRelay (后台 goroutine):
--     → SELECT * FROM data_change_outbox WHERE sent_at IS NULL ORDER BY id LIMIT N
--     → For each row:
--          (a) SET Redis key llm:cache:<table>:<pk> = payload (with TTL)
--          (b) PUBLISH Redis channel llm-gateway.invalidate {"table":...,"pk":...,"version":...}
--          (c) UPDATE outbox SET sent_at=NOW() WHERE id=?
--
--   gateway 监听 PUBSUB → 收到 → 清掉本地 L1 缓存（下次请求 L2 Redis 命中拿到 fresh 数据）
--
-- payload 列是 JSON 形态的实体快照（与 domain.X struct 字段一致）。delete 操作 payload 留空。
--
-- 保留期：定期清理 sent_at < NOW() - 7d 的行（OPS task；不在 schema 里强制）。
-- =====================================================================
CREATE TABLE IF NOT EXISTS data_change_outbox (
    id          BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
    table_name  VARCHAR(64)  NOT NULL,                       -- "model_services" | "endpoints" | ...
    op          VARCHAR(16)  NOT NULL,                       -- "upsert" | "delete"
    pk          VARCHAR(191) NOT NULL,                       -- 主键 string 形态（id 转字符串 / 复合键拼接）
    payload     JSON         DEFAULT NULL,                   -- upsert 实体 JSON 快照；delete 留空
    version     BIGINT UNSIGNED NOT NULL DEFAULT 0,          -- 单调递增；relay 顺序消费保证

    created_at  TIMESTAMP(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    sent_at     TIMESTAMP(6) NULL DEFAULT NULL,              -- relay 写到 Redis 成功后填

    INDEX idx_unsent (sent_at, id)                           -- 查未发送行；relay 主索引
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
