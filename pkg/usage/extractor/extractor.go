// Package extractor 定义 Usage 提取层：把上游响应转成 ctx.Usage。
//
// 一个 Extractor 对应一种"上游响应格式"，可被多个 Adapter 复用
// （如 OpenAI / Azure / DeepSeek 都用 openai_compat）。
//
// 详见 docs/architecture/05-metering-billing.md 第 4 节。
package extractor
...
// Extractor 工厂：每次请求新建一个 Session。
type Extractor interface {
	Name() string
	NewSession(c context.Context, meta ctx.UsageMeta) Session
}
