...
// GetByAPIKeyID 按 (account_id, api_key_id) 查未删 key。
//
// 注意：Get 走 api_key_id（稳定审计 ID），不走明文 / hash——后者是凭证，
// admin 不该用它做 lookup。
func (s *APIKeyStore) GetByAPIKeyID(ctx context.Context, accountID, apiKeyID string) (*repo.APIKey, error) {
	if accountID == "" || apiKeyID == "" {
		return nil, errors.New("apikey: account_id and api_key_id required")
	}
	var k repo.APIKey
	if err := s.db.WithContext(ctx).
		Where("account_id = ? AND api_key_id = ? AND deleted_at IS NULL", accountID, apiKeyID).
		First(&k).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, fmt.Errorf("apikey: not found: account=%s api_key_id=%s", accountID, apiKeyID)
		}
...
// List 列主账号范围内未删 keys；可按 sub_account_id / enabled 过滤（空 / nil 表示不过滤）。
func (s *APIKeyStore) List(ctx context.Context, accountID, subAccountIDFilter string, enabledFilter *bool) ([]repo.APIKey, error) {
	if accountID == "" {
		return nil, errors.New("apikey: account_id required")
	}
	q := s.db.WithContext(ctx).Where("account_id = ? AND deleted_at IS NULL", accountID)
	if subAccountIDFilter != "" {
		q = q.Where("sub_account_id = ?", subAccountIDFilter)
	}
...
//   - api_key_id：`ak_<sub_account_id>_<6字节 hex>`
...
func (s *APIKeyStore) Create(ctx context.Context, k *repo.APIKey) (plaintext string, err error) {
	if k == nil || k.SubAccountID == "" {
		return "", errors.New("apikey: sub_account_id required")
	}
	if k.AccountID == "" {
		k.AccountID = "default"
	}
	...
	if k.APIKeyID == "" {
		id, err := generateAPIKeyID(k.SubAccountID)
		if err != nil {
...
// Update 按 (account_id, api_key_id) 更新可变字段：enabled / expires_at /
// group / external_user / name。
//
// **api_key_hash / api_key_prefix 不可改**——要换 key 必须 Delete + Create
// （这正是设计意图：凭证泄露后只能 rotate 不能 patch）。
func (s *APIKeyStore) Update(ctx context.Context, accountID, apiKeyID string, updates APIKeyUpdates) error {
	if accountID == "" || apiKeyID == "" {
		return errors.New("apikey: account_id and api_key_id required")
	}
	...
	if res.RowsAffected == 0 {
		return fmt.Errorf("apikey: not found: account=%s api_key_id=%s", accountID, apiKeyID)
	}
	return nil
}
...
func (s *APIKeyStore) Revoke(ctx context.Context, accountID, apiKeyID string) error {
	if accountID == "" || apiKeyID == "" {
		return errors.New("apikey: account_id and api_key_id required")
	}
	now := time.Now().UTC()
...
func (s *APIKeyStore) Delete(ctx context.Context, accountID, apiKeyID string) error {
	if accountID == "" || apiKeyID == "" {
		return errors.New("apikey: account_id and api_key_id required")
	}
	now := time.Now().UTC()
...
// generateAPIKeyID `ak_<sub_account_id>_<12hex>`。subAccountID 太长截断 24 字节避免 VARCHAR(64) 溢出。
func generateAPIKeyID(subAccountID string) (string, error) {
	b := make([]byte, 6)
	if _, err := rand.Read(b); err != nil {
		return "", err
	}
	if len(subAccountID) > 24 {
		subAccountID = subAccountID[:24]
	}
	return fmt.Sprintf("ak_%s_%s", subAccountID, hex.EncodeToString(b)), nil
}
