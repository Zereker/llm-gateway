...
// 索引 idx_active_lookup (account_id, model_service_id, rule_class, effective_from)
// 覆盖热路径；活跃行查询 ~1ms。
//
...
const pvColumns = `id, account_id, model_service_id, rule_class,
...
func (p *SQLPricingProvider) GetActive(ctx context.Context, accountID string, modelServiceID int64, ruleClass string, t time.Time) (*PricingVersion, error) {
	if accountID == "" {
		return nil, errors.New("pricing: empty account_id")
	}
	if modelServiceID == 0 {
	...
	err := p.db.GetContext(ctx, &pv, p.db.Rebind(
		...
		accountID, modelServiceID, ruleClass, t, t,
	)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, fmt.Errorf("pricing: no active version for account=%s model_service_id=%d class=%s at %s",
				accountID, modelServiceID, ruleClass, t.UTC().Format(time.RFC3339))
		}
...
func (p *SQLPricingProvider) ListHistory(ctx context.Context, accountID string, modelServiceID int64, ruleClass string) ([]*PricingVersion, error) {
	if accountID == "" {
		return nil, errors.New("pricing: empty account_id")
	}
	if modelServiceID == 0 {
	...
	if err := p.db.SelectContext(ctx, &rows, p.db.Rebind(
		...
		accountID, modelServiceID, ruleClass,
	); err != nil {
