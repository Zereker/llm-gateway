...
//
// 多主账号 + 多规则：同一 model_service 在不同主账号 / rule_class 下可有完全
...
type PricingProvider interface {
	// GetActive 返回 (account, model_service, rule_class) 在 t 时刻活跃的价格版本。
	//
	// 活跃 = effective_from <= t AND (effective_to IS NULL OR effective_to > t)
	//
	// 找不到时返回带充分上下文的错误（M5 直接把 err.Error() 写进 abort message）。
	GetActive(ctx context.Context, accountID string, modelServiceID int64, ruleClass string, t time.Time) (*PricingVersion, error)

	// ListHistory 列某 (account, model_service, rule_class) 的全部历史版本。
	// 按 effective_from 倒序，最新在前。admin UI 用。
	ListHistory(ctx context.Context, accountID string, modelServiceID int64, ruleClass string) ([]*PricingVersion, error)
}
