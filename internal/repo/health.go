...
// a missed schema migration early, with a friendlier error than "SQL: no
// such table").
//
...
func CheckSchema(ctx context.Context, db *sqlx.DB) error {
	for _, t := range []string{
		...
		if _, err := db.ExecContext(ctx, "SELECT 1 FROM "+t+" LIMIT 0"); err != nil {
			return fmt.Errorf("repo: schema check failed on %q (run the embedded infra migrations first): %w", t, err)
		}
	}
