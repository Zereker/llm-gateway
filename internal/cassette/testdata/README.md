...
Package-owned fixtures used by cassette, gateway, and black-box tests. The bulk
real-traffic corpora are *not* here: they come from
...
- `internal/app/gateway`'s `TestE2E_MultiVendor_AllProtocols` and the
  `examples/support/mockupstream` smoke test — a mock upstream replays a real captured body
...
- `examples/support/seed-fieldmatrix` — seeds a real MySQL instance for
  `examples/support/e2e/smoke-multivendor.sh`'s real-binary (`cmd/gateway` +
  `examples/support/mockupstream`) black-box run.

Adding a vendor to *both* is one new JSON file here — see any existing file for
the shape, and `examples/support/mockupstream`'s doc comment for which `upstream_path` values
it actually serves.
...
git diff internal/cassette/testdata/fieldmatrix/golden/   # read it by hand before committing
```
