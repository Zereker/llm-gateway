// Package scenario loads the standard request-body packs under
// testdata/record-scenarios/ (repo root) that scripts/record-cassette's
// batch mode replays against a real vendor API. A scenario is one JSON
// request body; the pack as a whole is what guarantees a self-recorded
// cassette set covers the same parameter surface a real SDK exercises —
// see that directory's README for each body's provenance, and this
// package's test for the coverage rules the pack must keep satisfying.
package scenario
