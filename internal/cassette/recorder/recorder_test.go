...
// against a real (test) HTTP server, writes the cassette, and loads it back
// through the actual internal/cassette.Load — proving the write side and
// the read side agree on the on-disk format, and that no trace of the API
// key survives anywhere in the file.
...
func TestAppendAcrossRuns(t *testing.T) {
	...
	if string(its[1].RequestBody) != `{"n":2}` {
		t.Errorf("appended request body mismatch: %q", its[1].RequestBody)
	}
}
