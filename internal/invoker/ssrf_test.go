...
func TestIsMetadataIP(t *testing.T) {
	cases := []struct {
		...
		{"100.100.100.200", true, "Alibaba Cloud ECS metadata (exact /32; rest of CGNAT stays allowed)"},
		{"192.0.0.192", true, "Oracle Cloud classic metadata (IETF-reserved 192.0.0.0/24, not link-local)"},
		{"100.100.100.201", false, "adjacent CGNAT addr -- only the exact Alibaba metadata IP is blocked, not the /10"},
		{"100.64.0.1", false, "CGNAT range base -- self-hosted upstream on shared address space stays allowed"},
		{"192.0.0.193", false, "adjacent to Oracle metadata -- only the exact /32 is blocked"},
