...
// by the manifests under internal/cassette/testdata/fieldmatrix/endpoints/ — see
// internal/cassette/vendorfixture's doc comment for the file shape. A
// freshly started local stack (make dev-up + make run-mockupstream + make
...
// in-process — letting a human (or examples/support/e2e/smoke-multivendor.sh) drive a
...
// Usage:
//
//	go run ./examples/support/seed-fieldmatrix \
...
func main() {
	...
	mockBase := flag.String("mock-base", "http://127.0.0.1:9090", "mockupstream base URL (see examples/support/mockupstream's route table)")
	flag.Parse()
