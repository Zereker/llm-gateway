...
# examples/support/mockupstream binaries over a real network round trip, so a genuine
...
#   3. go run examples/support/mockupstream (bg)          fake upstream: openai/anthropic/gemini/cohere/azure-openai/bedrock routes
#   4. go run cmd/gateway     (bg)            data plane (runs infra.Migrate at startup)
#   5. go run examples/support/seed-fieldmatrix        one endpoint + one real api_key PER VENDOR (idempotent)
#   6. curl /v1/chat/completions or /v1/messages, once per vendor -> expect 200 + content
#   7. cleanup (kill background pids)
#
# Usage:
#   ./examples/support/e2e/smoke-multivendor.sh                # default 60s timeout; keeps the docker stack
#   ./examples/support/e2e/smoke-multivendor.sh --teardown     # runs docker compose down -v afterward
#   ./examples/support/e2e/smoke-multivendor.sh --no-docker-stack  # assume mysql/redis are already
...
#
# Exit codes:
#   0  all vendors passed (see internal/cassette/testdata/fieldmatrix/endpoints/*.json for the current list)
...
cd "$REPO_ROOT"
COMPOSE=(docker compose -p llm-gateway-dev -f examples/local/compose.yaml)
...
if [[ "$NO_DOCKER_STACK" == "0" ]]; then
  ...
  for ((i=0; i<30; i++)); do
    if "${COMPOSE[@]}" exec -T mysql mysqladmin ping -h localhost -uroot --silent >/dev/null 2>&1; then
      break
    fi
    sleep 1
  done
...
MOCK_ADDR=":$MOCK_PORT" go run ./examples/support/mockupstream >"$LOG_UP" 2>&1 &
PID_UP=$!
...
go run ./cmd/gateway -config ./examples/local/configs/gateway.yaml >"$LOG_GW" 2>&1 &
PID_GW=$!
...
# matches internal/cassette/testdata/fieldmatrix/endpoints/ exactly, with no per-vendor blocks
# to keep in sync by hand.
SEEDED="$(go run ./examples/support/seed-fieldmatrix \
  -dsn "$DSN" \
  -data-key "$DATA_KEY" \
...
# repo TTL cache defaults to 30s; see examples/support/e2e/smoke.sh's identical note.
sleep 2
