...
#   3. go run examples/support/mockupstream (bg)   fake upstream
#   4. go run cmd/gateway     (bg)    data plane (runs infra.Migrate at startup)
#   5. go run examples/support/seed-demo        writes minimal business data to the DB (account / endpoint / api_key)
#   6. curl /v1/chat/completions      expect 200 + content
#   7. cleanup (kill background pids)
#
# Usage:
#   ./examples/support/e2e/smoke.sh                # default 60s timeout; keeps the docker stack
#   ./examples/support/e2e/smoke.sh --teardown     # runs docker compose down -v afterward
#
# Exit codes:
#   0  everything passed
...
go run ./examples/support/seed-demo \
  -dsn "$DSN" \
  -data-key "$DATA_KEY" \
