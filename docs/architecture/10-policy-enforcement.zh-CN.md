...
JSON `AuditRecord` 不包含内容字节、替换值和内部错误原因。
mutation 审计只记录 ID、kind 和 RFC 6901 target。显式策略引擎
绝不向客户端暴露 `Decision.Cause`；只有旧版适配器为兼容性保留
原有的输入审核错误消息。
...
继续通过 `moderation.LegacyEngine` 获得支持。新的集成应实现
