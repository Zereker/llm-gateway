...
| Sender (`pkg/invoker`) | 拿 Handler 跑 PrepareCall + HTTP Do + 响应 forward + 错误归类（不做协议查找——dispatch 已经通过 `protocol.Lookup` 拿到 Handler） |
...
pkg/dispatch/eligibility.go
    eligibility.go             纯函数 Filter()：protocol.Lookup 拿不到 Handler / 模态不支持 → 剔除
...
4. `pkg/dispatch/eligibility.go`（纯函数 60 行）
