...
## 16. 已知限制（安全 / 演进债，动手前先读）

- **API key hash 是无盐单轮 SHA-256**（`repo.HashAPIKey`）：api_keys 表泄漏时可离线
  暴破弱 key / 跨账号关联相同 key。gateway 不生成 key（deployer 插 hash），无法保证
  key 熵。缓解：deployer 生成 ≥256bit 随机 key。根治方向：HMAC-SHA256(pepper, key)
  ——需要 dual-read 迁移（老 hash + 新 hash 并查一段时间），未排期。
- **data_key（KEK）无轮换路径**：密文带 `v1:` 前缀但没有 v2 解密链——怀疑 KEK 泄漏时
  只能停机手工 decrypt/re-encrypt 全部 endpoints.auth。根治方向：`SetDataKeys(new, old...)`
  多 key 解密 + 后台 re-encrypt，未排期。
- **限流不兼容 Redis Cluster**：见 [04 §7a](./04-rate-limiting.md#7a-redis-部署形态限制)。
- **OTel Baggage 不得注入上游请求**：内部租户标识在 baggage 里；上游 client 只允许
  注入 traceparent（见 `pkg/trace/otel.go` 注释）。
