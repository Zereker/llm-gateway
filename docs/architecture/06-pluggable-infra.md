...
3. **CDC stream**：deployer 改的 SQL 行经 Debezium 捕获 binlog 后写到 Redis Stream
...
## 8. CDC：SQL → gateway 数据传播

gateway 自跑 `infra.Migrate` 建表；业务行由 deployer SQL 维护。
两边的连接桥不能是直连同库 + 每请求查表——QPS 起来后 MySQL 是瓶颈。目标方案：

```text
deployer --SQL INSERT/UPDATE--> MySQL --binlog--> Debezium Server --topic--> Redis Streams
                                                                  │
                                                                  ▼
...
随 deployer 写频率提升按需接入；接入模式相同，每张表 ~30 行胶水代码（cache 实例 + 适配
...
- 风险窗口：deployer 改了某行，gateway L1 还缓存着旧值，直到该 LRU 条目自然淘汰或
...
- **不做 schema 演进 hook**：deployer 执行 schema.sql 落表后 binlog 自然带新字段；
...
- 新接 CDC 的表（§8）必须同步：定义 `TieredCache[T]` 实例、注册到 `StreamConsumer`、
  在 deployer 侧确认 Debezium 已 capture 对应表、本文档 §8.4 接入表清单。
