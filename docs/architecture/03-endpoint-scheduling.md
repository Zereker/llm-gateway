...
dispatch 使用 `dispatch.Class`（外层 Verdict 字段），selector 内部使用同语义的
`selector.ErrorClass`；两者在 `pkg/dispatch/adapters/` 双向映射（dispatch→selector
方向在 `adapters/selector.go`，selector→dispatch 方向在 `adapters/invoker.go` 的
`selectorClassToDispatch`），
保持 dispatch 不依赖 selector 类型、selector 不依赖 dispatch 类型：
...
## 8. Runtime Scoring（已实现，默认关）

**实现状态**：已落地（`pkg/selector` DefaultScorer + EndpointStatsStore），`scoring.enabled`
默认关；开启后 `Scheduler.Report` 写 EMA 统计、`Pick` 时按 success/latency factor 调
`EffectiveWeight`。stats 存储有两个 driver：`inmemory`（每副本独立）/ `redis`
（多副本共享，打分一致）——见 07-configuration `scoring:` 段。以下是设计原则。

当前默认调度只使用静态 `endpoint.weight`。这简单可控，但没有把运行时质量量化进选择过程：
