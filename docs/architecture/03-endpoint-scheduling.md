...
| `pkg/dispatch/eligibility.go`（dispatch 内部 helper，不是独立 package） | 纯函数资格过滤：`protocol.Lookup` 拿不到 Handler / 模态不支持 → 剔除 |
| `pkg/invoker` | 拿 Handler 跑 PrepareCall + HTTP Do + 响应 forward + 错误分类（不做协议查找——协议层 Lookup 在 dispatch 内） |
