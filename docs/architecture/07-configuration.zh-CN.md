...
Gateway 启动时应用待执行版本，并记录到 `schema_migrations`。
首个 release 之前，所有 Schema 变更都压缩到
`internal/infra/migrations/000001_base.sql`；携带旧 pre-release 历史的数据库需要重建。
首个 release 之后，已合入的迁移文件不可修改，Schema 演进只能新增下一个编号文件，
CI 会拒绝修改或删除已有迁移。

副本并发启动时迁移保持幂等和安全，因此 Gateway 数据库用户需要相应 DDL 权限。
迁移与 Schema 校验受 30 秒启动期限约束，元数据锁竞争会导致启动失败而不是无限等待。
破坏性变更使用 expand/migrate/contract，并必须在不兼容应用代码发布前完成，参见
