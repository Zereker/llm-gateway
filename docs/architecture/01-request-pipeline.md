...
| M5 | ModelService | `rc.Envelope.Model`、主账号 pin | 原始请求 model 的 `rc.ModelService` |
...
M5 使用 middleware-owned interfaces，由 `pkg/cdc.TieredCache` 适配后注入：

- `ModelCatalog.GetByModel` 查全局 model catalog（生产实现：L1 LRU 命中直接返回；
  miss 走 L3 loader = `repo.SQLModelServiceReader.GetByModel`；CDC event 触发 L1 失效）。
- `SubscriptionChecker.HasModel` 校验主账号是否订阅该 model。

`model_services` 是全局 catalog，不再按主账号存储；主账号可见性由 `account_model_subscriptions` 表决定。

`SubscriptionChecker` 等其余 5 张表的 CDC 接入按需做（详见 [06 §8.4](./06-pluggable-infra.md#84-适用表)）；未接入的走 SQL 直查 + 上层 TTL 缓存。

M5 不查询 active pricing，也不向 `RequestContext` 写 pricing snapshot。价格匹配和金额计算由下游计费平台根据 Usage Event 的请求发生时间完成。

跨 model fallback 不复用原始请求的 M5 结果。M7 对每个 fallback model 重新调用 M5 所属接口做 catalog + subscription 校验；成功选中某个 model 后写 `rc.RoutedModelService`。usage meta 和 attempt 记录都使用实际路由 model。
